...
/// The main timer clock interface — Tab 1.
///
/// Tapping "Top-Up" or "Consume" presents an activity picker sheet.
/// Selecting an activity starts the respective timer session.
struct TimerView: View {
    @EnvironmentObject var timeManager: TimeManager

    /// Controls which category picker sheet is presented (nil = no sheet).
    @State private var sheetCategory: AppState? = nil
    ...
    var body: some View {
        ...
        .sheet(item: $sheetCategory) { category in
            ActivityPickerSheet(category: category) { profile in
                sheetCategory = nil
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    switch profile.category {
                    case .toppingUp:
                        timeManager.startTopUp(with: profile)
                    case .consuming:
                        timeManager.startConsume(with: profile)
                    case .idle:
                        break
                    }
                }
    ...
    private var clockView: some View {
        ZStack {
            ...
            VStack(spacing: 4) {
                ...
                if let name = timeManager.activeActivityName {
                    Text(name)
    ...
    private var controlButtons: some View {
        HStack(spacing: 20) {
            ...
            ActionButton(
                ...
                if timeManager.currentState == .toppingUp {
                    timeManager.stopTimer()
                } else {
                    sheetCategory = .toppingUp
                }
            }
            ...
            ActionButton(
                ...
                if timeManager.currentState == .consuming {
                    timeManager.stopTimer()
                } else {
...
// MARK: - Make AppState Identifiable for .sheet(item:)
...
/// Half-sheet that displays activities for a given category.
struct ActivityPickerSheet: View {
    @EnvironmentObject var timeManager: TimeManager
    let category: AppState
    let onSelect: (ActivityProfile) -> Void

    private var profiles: [ActivityProfile] {
        category == .toppingUp ? timeManager.topUpProfiles : timeManager.consumeProfiles
    }

    private var accentColor: Color {
        category == .toppingUp ? Color(hex: 0x00E5A0) : Color(hex: 0xFC466B)
    }
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationTitle(category == .toppingUp ? "Select Top-Up Activity" : "Select Consume Activity")
